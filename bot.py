...
def retry_call(fn, *args, retries=4, base_sleep=2, **kwargs):
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            print(f"[API RETRY] {attempt}/{retries} -> {short_err(e)} | sleep {wait:.1f}s")
            time.sleep(wait)
        except Exception as e:
            wait = base_sleep + random.uniform(0, 0.5)
            print(f"[API RETRY] -> {short_err(e)} | sleep {wait:.1f}s")
            time.sleep(wait)
    return None
...
def fetch_trade_fees(api_key: str, api_secret: str) -> tuple[Decimal, Decimal]:
    ...
    try:
        ...
        print("[FEE] tradeFee çekilemedi, fallback:", short_err(e))
        return DEFAULT_MAKER_FEE, DEFAULT_TAKER_FEE
...
def main():
    ...
    if step is None:
        print("[FATAL] exchange_info alınamadı.")
        return
            ...
            if not holding:
                ...
                if px_strategy < bb_lower:
                    ...
                    if use < MIN_TRADE_QUOTE:
                        ...
                        if expected_edge <= roundtrip_cost:
                            ...
                            action_text = f"{tr_num(float(use),2)} {QUOTE_ASSET} ALIM"
                            planned_text = "-"
                # log
                eq2 = bot_equity(state, px_live)
                pnl_usdt_total = eq2 - start_eq
                ...
                if px_strategy > bb_mid:
                    if sell_qty * px_live >= min_notional and sell_qty > 0:
                        ...
                        action_text = f"{tr_num(float(proceeds),2)} {QUOTE_ASSET} SATIM"
                        planned_text = "-"
                # log
                eq2 = bot_equity(state, px_live)
                pnl_usdt_total = eq2 - start_eq
